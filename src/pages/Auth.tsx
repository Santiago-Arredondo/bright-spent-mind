...
const Auth = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes("already")) toast.error(t("auth_already"));
      else if (msg.toLowerCase().includes("invalid")) toast.error(t("auth_bad_creds"));
      else toast.error(msg || t("save_error"));
    } finally {
      setBusy(false);
    }
  };
        ...
        <div className="w-full max-w-md bg-card border border-border rounded-3xl shadow-card p-8">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <Button
              type="submit"
              disabled={busy}
              className="w-full rounded-xl h-11 bg-gradient-primary hover:opacity-90 shadow-glow"
            >
              {busy ? t("saving") : mode === "signin" ? t("auth_signin_cta") : t("auth_signup_cta")}
            </Button>
          </form>
          ...
          <p className="text-sm text-center text-muted-foreground mt-6">
            ...
            >
              {mode === "signin" ? t("auth_signup_cta") : t("auth_signin_cta")}
            </button>
          </p>
        </div>
      </main>
